...
public class MsgDbHelper {
	...
	public List<ChatItem> getChatMsg(String chatName){
		...
		ChatItem msg;
		String sql = "select a.chatType,a.chatName,a.nickName,a.username,a.head,a.msg,a.sendDate,a.inOrOut " +
		...
		while(cursor.moveToNext()){
			msg = new ChatItem(cursor.getInt(0),cursor.getString(1),cursor.getString(2),cursor.getString(3), cursor.getString(4), cursor.getString(5)
	...
	public int getChatMsgsize(String chatName){
		...
		String sql ="select a.chatType,a.chatName,a.nickName,a.username,a.head,a.msg,a.sendDate,a.inOrOut " +
				" from(select * from "+helper.DB_NAME +
				" where chatName = ? and whos = ?"+")a order by a.id";
		Cursor cursor = db.rawQuery(sql, new String[]{chatName,Constants.USER_NAME});
	...
	public List<ChatItem> getLastMsg(){
		List<ChatItem> chatItems = new ArrayList<>();
		ChatItem msg;
		String sql ="select chatType,chatName,nickName,username,head,msg,sendDate,inOrOut from  "+helper.DB_NAME +
				" where whos = ? "+
				 " GROUP BY chatName "+
					"order by id desc";
